...
| **Tara Donnolley** — new professional headshot | Current shot is fine; you flagged a fresh pro headshot for the next round. Replace `src/assets/team/tara.jpg` when ready. | `/team`, `/remedial-massage`, `/locations/currimundi` |
| **Suzie Irwin** — refreshed professional headshot | Current pro shot is in. You flagged wanting another professional headshot for the next round when one's available. Replace `src/assets/team/suzie.jpg`. | `/team`, `/remedial-massage`, `/locations/currimundi` |
...
- **Hydrotherapy hero photo** — *you said you'd source a better one.* Interim stock image wired in for now (Pexels free-licence indoor-pool shot, older woman + younger woman in the water, at `src/assets/services/hydrotherapy.jpg` + `/services` card uses the same file). When you have something better (real Bribie Island Aquatic Centre shot or a physio-in-pool image), drop it in `/Users/joe/Downloads/` and tell me the filename, I'll swap it in.
